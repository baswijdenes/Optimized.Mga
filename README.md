...
The main cmdlet is of course Batch-Mga, but using Fiddler, of the browser developer tools you can find the URL when navigating through AzureAD and use it in one of the cmdlets. 
...
### Bulk
Patch-Mga with -InputObject and the Property members@odata.bind with 20+ users, will automatically be batched. So, in theory you can add 10000 users to a Group instantly.
```PowerShell
...
Same goes for Delete-Mga. When parameter -URL is an Array of over 20+ it will automatically batch your request:
```PowerShell
...
For example, I used this in the beta version to get the latest login. Nowadays this property is with the user under signInActivity.
